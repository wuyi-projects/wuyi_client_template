<template>
  <div class="app-container">
    ...
    <el-card class="box-card" shadow="never" style="margin-bottom:16px;">
      <el-form ref="searchForm" :model="searchFormData" :rules="rules" label-width="120px" size="small">
    ...
    <el-card class="box-card" shadow="never">
      ...
      <vxe-grid ref="dataGrid" class="custom-table-scrollbar" v-bind="gridOptions">

        <!--工具栏按钮-->
        <template v-slot:buttons>
          <el-button-group>
            <el-button @click.native.prevent="handleCreate()">新增</el-button>
            <el-button type="primary" @click.native.prevent="handleBatchDelete()">批量删除</el-button>
          </el-button-group>

          <el-button-group style="margin-left:10px;margin-bottom: 3px;">
            <el-button @click.native.prevent="handleDownload()">模板下载</el-button>
            <el-button type="primary" @click.native.prevent="handleImport()">Excel导入</el-button>
          </el-button-group>
        ...
        <template v-slot:operate="{ row }">
          <el-button type="text" @click="handleManagePermissionGroup(row)">权限分组管理</el-button>
    ...
    >
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="100px" style="width: 100%; padding:10px;">
        <el-row>
          <el-col :span="24">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" :loading="loadingSubmitButton" :disabled="loadingSubmitButton" @click="dialogStatus==='create'?createData():updateData()">
          {{ submitButtonText }}
        </el-button>
      </div>
    </el-dialog>
        ...
        <el-col :span="24" style="padding:0 20px 20px;">
          ...
          <el-row>
            <el-col :span="24">
              ...
              <el-button v-if="currentGroupAllPermissioms.length>0" type="text" style="margin-left:10px;" @click="test()">[清除全部]</el-button>
            </el-col>
            <el-col v-if="currentGroupAllPermissioms.length<1" :span="24" style="margin-top: 10px">
              <el-alert
                title="暂无下属权限"
                type="info"
                :closable="false"
            ...
            <el-col v-for="permission in currentGroupAllPermissioms" :key="permission.id" :span="8" style="margin-top: 10px">
              <el-tag closable @close="handleClearGroupInfo(permission)">{{ permission.description }}</el-tag>
            </el-col>
          </el-row>
          <el-row style="margin-top:20px;">
            <el-col :span="24">
              <vxe-grid ref="permissionGrid" class="custom-table-scrollbar" v-bind="permissionGridOptions">

                <!--工具栏按钮-->
                <template v-slot:toolbar_buttons>
                  <el-button type="primary" @click="handleBatchSetGroupInfo">批量加入分组</el-button>
                </template>

                <template v-slot:tools>
                  <el-form ref="permissionForm" :inline="true" :model="formInline" class="demo-form-inline">
                    <el-form-item style="margin-bottom: 0;">
                      <el-input v-model="searchPermissionFormData.permission" placeholder="输入权限名或代码模糊查询" clearable />
                    </el-form-item>
                    <el-form-item style="margin-bottom: 0;">
                      <el-button type="primary" @click="submitPermissionForm('permissionForm')">查询</el-button>
                    </el-form-item>
                  </el-form>
                </template>
                ...
                <!--数据行操作-->
                <template v-slot:operate="{ row }">
                  <el-button type="text" @click="handleSetGroupInfo(row)">加入分组</el-button>
                </template>
                <!--自定义空数据模板-->
                <template v-slot:empty>
                  <span>
...
import { listPermissiomGroupInfoByPage, savePermissiomGroupInfo, batchDeletePermissiomGroupInfo, deletePermissiomGroupInfo, updatePermissiomGroupInfo, listPermissioms4UngroupedWithPage, listAllPermissioms4Group, setGroupInfo4Permission, setGroupInfo4Permissioms, clearGroupInfo4AllPermissioms, clearGroupInfo4Permission } from '@/api/permission'
...
export default {
        ...
        proxyConfig: {
          ...
          ajax: {
            ...
            query: ({ page, sort, filters }) => {
              ...
              return listPermissioms4UngroupedWithPage(result)
            }
          }
        },
        toolbar: {
        ...
        proxyConfig: {
          ...
          ajax: {
            ...
            query: ({ page, sort, filters }) => {
              ...
              const result = Object.assign(pageData, searchData, sortParams)
              return listPermissiomGroupInfoByPage(result)
            }
          }
        },
        columns: [
  ...
  methods: {
    ...
    handleManagePermissionGroup(row) {
      ...
      this.listAllPermissioms4Group(row.id)
    ...
    handleDelete(row) {
      this.$confirm('永久删除该条记录吗, 是否继续?', '提示', {
        ...
        deletePermissiomGroupInfo(tempData)
          .then(response => {
    ...
    handleBatchDelete() {
      ...
      if (!(typeof selectRecords === 'undefined' || selectRecords === null || selectRecords === '' || selectRecords.length === 0)) {
        this.$confirm('永久删除记录吗, 是否继续?', '提示', {
          ...
          batchDeletePermissiomGroupInfo(batchDeleteData)
            .then(response => {
              const result = response.data
              if (result) {
                this.$message({
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          savePermissiomGroupInfo(tempData)
            .then(response => {
              const result = response.data
              if (result) {
                this.$message({
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          this.submitButtonText = '执行中...'
          const tempData = Object.assign({}, this.temp)
          updatePermissiomGroupInfo(tempData)
            .then(response => {
              const result = response.data
              if (result) {
                this.$message({
    ...
    listAllPermissioms4Group(currentGroupid) {
      listAllPermissioms4Group({
        permissionGroupInfoId: currentGroupid
      })
        .then(response => {
          const data = response.data
          console.log(JSON.stringify(data))
          this.currentGroupAllPermissioms = data
        })
        .catch(e => {
          this.loading = false
        })
    ...
    handleBatchSetGroupInfo() {
      ...
      if (!(typeof selectRecords === 'undefined' || selectRecords === null || selectRecords === '' || selectRecords.length === 0)) {
        this.$confirm('批量加入分组吗, 是否继续?', '提示', {
          ...
          setGroupInfo4Permissioms(batchSetGroupInfoData)
            .then(response => {
              const result = response.data
              if (result) {
                this.$message({
                ...
                this.listAllPermissioms4Group(this.currentGroupInfo.id)
                this.$refs.permissionGrid.commitProxy('reload')
              } else {
