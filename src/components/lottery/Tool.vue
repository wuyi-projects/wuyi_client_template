<template>
  <div id="tool">
    <el-button type="primary" size="mini" :disabled="disabledstart" round @click="startHandler">{{
      running ? '揭晓结果' : '开始抽奖'
    }}</el-button>
    <!-- <el-button size="mini" round @click="showRemoveoptions = true">
      重置抽奖
    </el-button> -->
    <!-- <el-button size="mini" @click="showImport = true" round>
      导入名单
    </el-button>
    <el-button size="mini" @click="showImportphoto = true" round>
      导入照片
    </el-button> -->
    <el-dialog
      :append-to-body="true"
    ...
    >
      <el-form ref="form" :model="form" label-width="80px" size="mini">
        ...
        <el-form-item v-if="form.category" label=" ">
          <span>
            共&nbsp;
        ...
        <el-form-item label="抽取方式">
          <el-select v-model="form.mode" placeholder="请选取本次抽取方式">
            <el-option label="抽1人" :value="1" />
            <el-option label="抽5人" :value="5" />
            <el-option label="一次抽取完" :value="0" />
            <el-option label="自定义" :value="99" />
          </el-select>
        </el-form-item>

        <el-form-item v-if="form.mode === 99" label="抽取人数">
          <el-input
            v-model="form.qty"
            type="number"
            :clearable="true"
